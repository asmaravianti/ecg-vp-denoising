...
from ecgdae.metrics import (
    ...
    compute_derivative_weights, evaluate_reconstruction, batch_evaluate
)
...
def load_model(model_path: str, config: dict, device: torch.device):
    ...
    # Load checkpoint
    # weights_only=False needed for PyTorch 2.6+ to load checkpoints with numpy objects
...
def compute_per_sample_metrics(
    ...
    prds = []  # PRD vs clean
    prds_vs_noisy = []  # PRD vs noisy (NEW)
    wwprds = []  # WWPRD vs clean
    wwprds_vs_noisy = []  # WWPRD vs noisy (NEW)
    snr_ins = []
    snr_outs = []
    snr_improvements = []
    ...
    for i in range(num_samples):
        ...
        # Compute metrics vs noisy (denoising improvement) - NEW
        prd_vs_noisy = compute_prd(n, r)
        wwprd_vs_noisy = compute_wwprd(n, r, w)

        prds.append(prd)
        prds_vs_noisy.append(prd_vs_noisy)
        wwprds.append(wwprd)
        wwprds_vs_noisy.append(wwprd_vs_noisy)
    ...
    result = {
        'PRD': np.array(prds),
        'PRD_vs_noisy': np.array(prds_vs_noisy),
        'WWPRD': np.array(wwprds),
        'WWPRD_vs_noisy': np.array(wwprds_vs_noisy),
...
def print_summary_table(metrics: Dict[str, np.ndarray]):
    """Print a summary table of metrics including PRD (vs clean) and PRD (vs noisy)."""
