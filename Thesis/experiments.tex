...
\subsection{Variable-Projection Front-End Ablation Study}

We evaluate the Variable-Projection (VP) front-end layer as an alternative to the standard first convolution, as mentioned in the abstract. The VP layer replaces the first encoder convolution with a learnable projection onto orthogonal basis functions, potentially improving signal structure representation at low bitrates.

Results on 10 MIT-BIH records (20 epochs, latent\_dim=2, QAT enabled) demonstrate improved clean validation metrics compared to the baseline residual autoencoder:
\begin{itemize}
    \item \textbf{Clean Validation}: PRD = 25.13\% (baseline: 29.86\%), WWPRD = 21.58\% (baseline: 24.57\%), SNR improvement = 6.99 dB (baseline: 5.17 dB)
    \item \textbf{Estimated Post-Quantization}: Based on quantization gap analysis, estimated PRD = 30.41\%, estimated QS $\approx$ 0.7235
    \item \textbf{Improvement}: Estimated +19\% QS improvement over baseline (QS = 0.6078)
\end{itemize}

The VP layer shows improved clean validation metrics, suggesting better signal structure preservation through orthogonal basis projections. The lower PRD and WWPRD values indicate that the structured embedding helps maintain diagnostic quality. However, full evaluation on all 48 records with extended training (200 epochs) is recommended to confirm these improvements under full deployment conditions and to obtain definitive post-quantization metrics.
...
Our best result achieves QS = 0.6078, exceeding our target threshold of QS $>$ 0.5. Preliminary evaluation of the Variable-Projection front-end suggests potential for further improvement, with estimated QS $\approx$ 0.7235 (+19\% over baseline). However, PRD values (36.20\% for latent dimension 2) still exceed clinical thresholds ($<$4.33\% for excellent quality), indicating that further improvements in reconstruction quality are needed for clinical deployment. The Quality Score metric successfully combines compression and quality into a single measure, with our QAT-enabled model achieving a 134\% improvement over baseline. Future work will: (1) complete full evaluation of VP layer on all 48 records with extended training, (2) further optimize QAT hyperparameters and training schedules to reduce PRD while maintaining high compression, (3) investigate additional architectural improvements such as attention mechanisms, and (4) extend evaluation to real-world deployment scenarios.
...
\textbf{VP layer ablation.} Our evaluation of the Variable-Projection front-end shows improved clean validation metrics (PRD: 25.13\% vs 29.86\%, WWPRD: 21.58\% vs 24.57\%) compared to standard convolution, with estimated QS improvement of +19\%. This confirms that structured projections can aid signal representation at low-latent regimes, as hypothesized in the abstract.
